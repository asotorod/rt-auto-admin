...
import {
  Users, Plus, Search, Filter, Phone, Mail, MessageCircle,
  Clock, Calendar, ChevronDown, ChevronRight, AlertCircle,
  Loader2, User, Car, X, RefreshCw, ArrowUpDown
} from 'lucide-react';
...
export default function LeadsList() {
  ...
  async function fetchLeads() {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          assigned_user:user_profiles!leads_assigned_to_fkey(id, first_name, last_name),
          vehicle:vehicles!leads_vehicle_interest_id_fkey(id, year, make, model, asking_price)
        `)
  ...
  const filteredLeads = leads.filter(lead => {
    ...
    return (
      ...
      (lead.vehicle_interest_text || '').toLowerCase().includes(q)
    );
  });
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchLeads} className="inline-flex items-center gap-2 px-4 py-2.5 bg-brand-card border border-brand-border rounded-lg text-brand-muted hover:text-white transition-colors">
            <RefreshCw size={16} />
          </button>
          <button className="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-gold text-brand-dark font-bold rounded-lg hover:bg-brand-gold-light transition-colors">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <button
          ...
          className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'all' ? 'bg-white/10 text-white border border-white/20' : 'bg-brand-card border border-brand-border text-brand-muted hover:text-white'}`}
        >
          All ({totalLeads})
        </button>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-brand-darker border border-brand-border rounded-lg pl-10 pr-4 py-2.5 text-white placeholder:text-brand-muted/50 focus:border-brand-gold/50 transition-colors"
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-muted hover:text-white">
                <X size={14} />
              </button>
            )}
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-brand-border">
                  ...
                  <th className="text-right px-5 py-3.5 text-xs font-semibold text-brand-muted uppercase tracking-wider">
                    <button onClick={() => { if (sortBy === 'created_at') setSortDir(d => d === 'desc' ? 'asc' : 'desc'); else { setSortBy('created_at'); setSortDir('desc'); } }} className="inline-flex items-center gap-1 hover:text-white transition-colors">
              ...
              <tbody className="divide-y divide-brand-border">
                {filteredLeads.map(lead => {
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-sidebar-hover transition-colors cursor-pointer group">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="text-sm text-white font-medium truncate">{lead.first_name} {lead.last_name || ''}</div>
                      ...
                      <td className="px-5 py-3.5 hidden lg:table-cell">
                        <div className="text-sm text-white truncate max-w-[200px]">
                          {lead.vehicle?.year ? (
                            <span className="flex items-center gap-1.5"><Car size={13} className="text-brand-muted flex-shrink-0" />{lead.vehicle.year} {lead.vehicle.make} {lead.vehicle.model}</span>
                          ) : lead.vehicle_interest_text ? (
                            <span className="text-brand-muted">{lead.vehicle_interest_text}</span>
                          ) : (
                            <span className="text-brand-muted/40">&mdash;</span>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-3.5 hidden md:table-cell">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-brand-muted capitalize">{lead.source?.replace('_', ' ')}</span>
                        </div>
                        ...
                        {lead.assigned_user ? (
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-brand-gold/20 flex items-center justify-center">
                              <span className="text-brand-gold text-[9px] font-bold">{lead.assigned_user.first_name?.[0]}{lead.assigned_user.last_name?.[0]}</span>
                            </div>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        ...
                        {lead.appointment_at && new Date(lead.appointment_at) > new Date() && (
                          <div className="text-[10px] text-brand-gold font-medium mt-0.5 flex items-center gap-1 justify-end">
                            <Calendar size={9} />Appt {new Date(lead.appointment_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                          </div>
                        )}
                      </td>
                    </tr>
